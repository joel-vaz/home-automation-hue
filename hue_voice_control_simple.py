...
class CommandProcessor(threading.Thread):
    ...
    def process_command(self, command):
        """Process voice commands and control the lights - simple on/off only"""
        if not self.bridge:
        ...
        try:
            ...
            # Process the command - ON/OFF only for non-dimmable lights
            if any(phrase in command for phrase in ["turn on", "lights on", "switch on", "power on", "on"]):
                logger.info("Turning lights ON")
                for light in targeted_lights:
                    light.on = True
                    
            elif any(phrase in command for phrase in ["turn off", "lights off", "switch off", "power off", "off"]):
                logger.info("Turning lights OFF")
                for light in targeted_lights:
                    light.on = False
            else:
                logger.info(f"Command '{command}' not recognized. Valid commands are 'turn on' or 'turn off'")
...
class HueVoiceControl:
    ...
    def run(self):
        """Main run method"""
        logger.info("Starting Hue Voice Control - Simple Version (ON/OFF only)")
        logger.info(f"Wake word: '{WAKE_WORD}' (say this before every command)")
        logger.info("For non-dimmable, non-color lights")
